...
export function SurfacesOverviewPreviewScreen() {
  const part = PREVIEW_PARTS.find((p) => p.id === 'IV');
  if (!part) return null;
  return (
    <Page>
      <Stamp
        number="IV"
        title="Surfaces"
        meta="banner · bed board · emr · telehealth · equipment · staff · registration"
      />
      <Sheet padding="md" className="max-w-[640px]">
        <ul className="m-0 flex list-none flex-col gap-3 p-0">
          {part.items.map((item) => (
            <li key={item.path}>
              <Link
                to={`/design-system/surfaces/${item.path}`}
