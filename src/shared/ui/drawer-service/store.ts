...
/**
 * Imperative store. Ported from
 * `ohlify/backend/packages/ui/src/modals/store.ts`. Singleton — feature code
 * never reaches for a provider. Mount `<ModalHost />` + `<ToastHost />` once
 * at the app root and call `DrawerService.toast(...) / .showFeedbackModal(...)`
 * from anywhere.
 */
type Listener = () => void;
